...
echo "====== 1. Kreiranje fajlova iz $file ======"
...
echo "====== 2. Checking format and sorting ======"
...
for f in *.kod; do
  if [[ $f =~ $regex ]]; then
    ...
    #   0 1 2 3 4 5 6 7 (hex_part indexi)
    ...
    if (( G_decimal % 2 == 0 )); then
      ...
      target_dir="${E_digit}0/${G_digit}0"
    else
      echo "$f - G digit is odd (decimal: $G_decimal)"
      X_decimal=$((G_decimal - 1))
      target_dir="${X_decimal}0/${E_digit}0"
    fi
